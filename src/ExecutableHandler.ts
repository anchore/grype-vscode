...
export class ExecutableHandler {
  ...
  private async handleGrypeArchiveStream(
    response: IncomingMessage
  ): Promise<void> {
    return new Promise((resolve, _) => {
      // Consider listening for errors on all streams and rejecting the promise in response
      response
        .pipe(zlib.createGunzip())
        .pipe(
          tar.extract({ cwd: this.storagePath } as tar.ExtractOptions, [
            "grype",
          ])
        )
        .on("close", resolve);
